...
- **Filter operators** (in query string `filter[]=` and `or[]=`): eq, gt, gte, in, lt, lte, ne, nin, like, ilike, isnull, notnull, contains, overlap. The last two (`contains`, `overlap`) are PostgreSQL-only (map to `@>` and `&&`); they will throw `SQLITE_ERROR` on SQLite.
...
- E2E tests use SQLite in-memory by default. Set `DATABASE_URL` env var or `TEST_DB=pg` for PostgreSQL.
- `yarn test` — SQLite (fast, no deps)
- `yarn test:pg` — PostgreSQL via testcontainers (requires Docker, auto-starts/stops container)
- `supportsArrayOperators` flag in `tests/utils` enables PG-only tests (`contains`, `overlap`).
- E2E test infra in `tests/utils/e2e.ts`: `prepareE2E()` auto-registers test entities (`Book`, `Page`, `Summary`, `Line`) and creates/drops schema per test file. For PG, calls `dropSchema()` + `createSchema()` per test.
